...
export function OrderDetails({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      // BUG: Sequential waterfall — each fetch waits for the previous one
      // Step 1: Fetch order
      const orderData = await fetchOrder(orderId);
      if (cancelled) return;
      setOrder(orderData);

      // Step 2: Fetch customer (needs customerId from order — this dependency is real)
      const customerData = await fetchCustomer(orderData.customerId);
      if (cancelled) return;
      setCustomer(customerData);

      // Step 3: Fetch shipping (only needs orderId — could have started earlier!)
      const shippingData = await fetchShippingStatus(orderId);
      if (cancelled) return;
